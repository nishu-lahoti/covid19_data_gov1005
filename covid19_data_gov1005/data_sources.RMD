...
economic_data <- global_gdp_cases %>% 
  full_join(stock_data_tidy, by = c("new_date" = "date", "CountryCode")) %>% 
  rename(pop_2018 = pop_2018.x) %>% 
  select(Country, CountryCode, new_date, sub.region, pop_2018, gdp_2018, gdp_per_capita, log_confirmed, log_deaths, log_recovered, stock, price)

# Saving file 

saveRDS(economic_data, file = "economic_data.RDS")
```
