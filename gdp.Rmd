...
# Downloading, cleaning, and merging JHU CSSE data for day-by-day case breakdown 
# CHECK IF NISHU HAS ALREADY DOWNLOADED THIS!!
...
confirmed <- read_csv("raw-data/time_series_19-covid-Confirmed.csv") %>% 
  ...
  rename("date" = "date")
# saveRDS(confirmed, file = "confirmed.RDS")
# confirmed <- readRDS("confirmed.RDS")
...
recovered <- read_csv("raw-data/time_series_19-covid-Recovered.csv") %>% 
  ...
  pivot_longer(names_to = "date",
               ...
               cols = -c(province, country)) %>% 
  rename("date" = "date")
# saveRDS(recovered, file = "recovered.RDS")
# recovered <- readRDS("recovered.RDS")

jhu <- confirmed %>% 
  inner_join(deaths) %>% 
  inner_join(recovered)
# saveRDS(jhu, file = "jhu.RDS")
```


```{r gdp_pop_data, include = FALSE}

# Get World Bank population data, most recent as of 2018
...
# Get World Bank GDP data, most recent as of 2018
...
citymapper <- download.file("https://cdn.citymapper.com/data/cmi/Citymapper_Mobility_Index_20200413.csv", destfile = "./data_misc/citymapper.csv", quiet = TRUE)

citymapper <- read_csv("data_misc/citymapper.csv", skip = 3)
...
#rscript
#readcsv takes link, reading in link 
#download.file
#good ways to handle rapidly updating data
